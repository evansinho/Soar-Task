...
const MyCards = () => {
  ...
  return (
    <div className="w-full md:p-0 p-3">
      <div className="flex justify-between items-center mt-2 mb-4 ml-2 md:ml-0 md:mt-0">
        <h2 className="text-lg md:text-xl lg:text-2xl text-[#343C6A] font-semibold">My Cards</h2>
        <Link to="/#" className="text-[#343C6A] text-sm md:text-base lg:text-lg hover:text-blue-500" aria-label="See All Cards">See All</Link>
      </div>
      <div className="flex align-center w-full gap-1 md:gap-1 lg:gap-2 xl:gap-8 overflow-x-auto">
        {dashboard.cards.map((card) => (
          <div key={card.id} className={`p-3 md:p-4 lg:p-5 rounded-3xl flex flex-col ${card.textColor} border border-gray-300 flex-grow min-w-[250px]`} style={{ background: card.bgColor }} aria-label={`Card ending in ${card.number.slice(-4)}`}>
              ...
              <div className="card-details-middle my-4 flex justify-between items-center">
                <div className="name">
                  <span className="text-xs md:text-xs">CARD HOLDER</span><br />
                  <span className="text-sm md:text-sm lg:text-base">{card.cardHolder}</span>
                </div>
                <div className="valid">
                  <span className="text-xs md:text-xs">VALID THRU</span><br />
